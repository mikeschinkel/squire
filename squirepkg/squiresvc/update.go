...
// UpdateRepoRequires updates the requires field in .squire/config.json for a repo
func UpdateRepoRequires(args *UpdateRepoRequiresArgs) (result *UpdateRepoRequiresResult, err error) {
	...
	if len(args.Config.ModuleSpecs) == 0 {
		args.Writer.Printf("Please add at least one module spec pattern using: squire project modspec add <pattern>\n")
		err = fmt.Errorf("no module_specs configured in ~/.config/squire/config.json")
		goto end
	}
	...
	store = cfgstore.NewProjectConfigStore(squire.ConfigSlug, squire.ConfigFile)
	store.SetConfigDir(repoRoot)

	// Check if config exists
	exists = store.Exists()
	if !exists {
		args.Writer.Printf("Repository not managed by Squire. Run 'squire init' first.\n")
