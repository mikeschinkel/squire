...
import (
	...
	"github.com/mikeschinkel/squire/squirepkg/goutils"
)
...
func (e *ReleaseEngine) Run(ctx context.Context) (result *EngineResult, err error) {
	...
	var leafModuleDir goutils.ModuleDir
	...
	e.graph = goutils.NewGraph(repoDir, goModFiles, goutils.ModuleGraphArgs{
		Logger: e.args.Logger,
		Writer: e.args.Writer,
	})
	err = e.graph.Build()
...
func (e *ReleaseEngine) getRepoModules(repoDir goutils.RepoDir) (modules []goutils.ModuleDir) {
	if e.graph == nil {
		return nil
	}
	...
	modules = make([]goutils.ModuleDir, 0, mods.Len())
...
func (e *ReleaseEngine) getSubmodulePathsToExclude(repoDir goutils.RepoDir, moduleDir goutils.ModuleDir) (excludePaths []dt.PathSegments) {
	if e.graph == nil {
		return nil
	}
...
func (e *ReleaseEngine) findLeafModule(ctx context.Context) (leafDir goutils.ModuleDir, err error) {
	var traverseResult *goutils.TraverseResult
	var leafCandidates []goutils.ModuleDir

	// Get the repo we're starting from
	_, ok := e.graph.ReposByRepoDir[e.graph.RepoDir]
	if !ok {
		err = NewErr(goutils.ErrNoGoModuleFound, "repo", e.graph.RepoDir)
		goto end
	}
	...
	err = NewErr(goutils.ErrNoGoModuleFound,
...
func (e *ReleaseEngine) isModuleInFlux(ctx context.Context, module *goutils.Module) (inFlux bool, err error) {
	var reason string
	var moduleExt *ModuleExt

	// Wrap base module in ModuleExt to access IsInFlux
	moduleExt = &ModuleExt{Module: module}
	err = moduleExt.SetGraph(e.graph)
	if err != nil {
		goto end
	}

	// Use ModuleExt.IsInFlux() method
	inFlux, reason, err = moduleExt.IsInFlux(ctx)
	if err != nil {
		goto end
	}
...
func (e *ReleaseEngine) hasDependenciesInFlux(ctx context.Context, module *goutils.Module) (hasInFluxDeps bool, err error) {
	var requireDirs []goutils.ModuleDir

	// Get the module directories that this module depends on
	requireDirs = module.RequireDirs()

	for _, depDir := range requireDirs {
		var depModule *goutils.Module
		var ok bool
		var inFlux bool
...
func (e *ReleaseEngine) computeVerdict(ctx context.Context, result *EngineResult) (err error) {
	...
	var modRelPath dt.PathSegments
	var report goutils.APIDiffReport
	...
	// Run API diff
	report, err = goutils.APIDiffDirs(baselineModuleDir, currentModuleDir, goutils.APIDiffDirsOptions{
