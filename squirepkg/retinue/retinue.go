...
//
// If the repo's .squire/config.json contains a requires field, DiscoverModules
...
func DiscoverModules(rootDir string) (ms *ModuleSet, err error) {
	...
	for len(queue) > 0 {
		...
		for _, req := range repoConfig.Requires {
			...
			// Check if repo has .squire/config.json
			...
			if !reqStore.Exists() {
				// Skip repos without .squire/config.json
				continue
			}
...
func discoverSingleRepoModules(repoRoot dt.DirPath, ms *ModuleSet) (err error) {
	...
	err = store.LoadJSON(&repoConfig)
	if err != nil {
		err = fmt.Errorf("failed to load .squire/config.json at %s: %w", repoRoot, err)
		goto end
	}
