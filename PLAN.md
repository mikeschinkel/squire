...
- Do **not** call or modify `go-nextver`; migrate needed logic into `squire` (likely under `retinue`). Leave `go-nextver` intact.
...
3) **Verdict Logic Migration**
   - Move needed nextver logic into `squire` (e.g., under `retinue`); do not call/modify `go-nextver` package.
...
- New/updated helpers in `retinue` for git/replace/in-flux and verdict logic (migrated from nextver)
...
1. **Copied packages from go-nextver**:
...
☒ Copy apidiffr package from go-nextver into squirepkg/apidiffr/
