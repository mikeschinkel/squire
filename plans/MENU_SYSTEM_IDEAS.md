...
- Saving Staging State — I want to explore having Staging record the current staging status and write to a staging.json file in the project config directory. I don't want ot do this immediately, but I want to have it in mind so that we can have functionality like git stash but for staging so that Gomion can save the staging state, allow the user to fiddle around with it, and then restore it in case they made an error. It should actually record this stage when it first loads so that it will always be there for a user to recover and not require them to have the forethought to save in advance. Further, it should be able to record multiple saves and be easy to manage, especially ones that are no longer relevant (unlike `git stash` where there are no tooling to manage old stashes.)
