...
// 1. Disposition keys (c/o/g/e) - We handle, emit changeDispositionMsg
...
// maybeChangeDispositionMsg returns a changeDispositionMsg for the currently selected file.
func (m FilesTableModel) maybeChangeDispositionMsg(disp FileDisposition) (msg changeDispositionMsg) {
	cursor := m.table.GetHighlightedRowIndex()
	if cursor < 0 {
		goto end
	}
	...
	msg = maybeChangeDispositionMsg(m.dir.Files[cursor].Path, disp)
