...
func (es EditorState) updateFileSelection(ctx context.Context, msg tea.Msg) (_ EditorState, cmd tea.Cmd) {
	...
	case changeDispositionMsg:
		// Handle disposition changes from table view
		// Map is updated in-place via pointer, so UI will reflect changes automatically
		es = es.SetDisposition(msg.RelFilepath, msg.Disposition)
		return es, scheduleSaveCmd
