...
import (
	...
	_ "github.com/mikeschinkel/gomion/gommod/gomcmds"
)
...
// Initialize sets up the gomion package with the provided options
...
func Initialize(args *RunArgs) (err error) {
	...
	gomion.SetLogger(args.Logger)

	if args.Logger == nil {
		err = errors.New("gommod.Initialize: Logger not set")
		goto end
	}
	...
	if err != nil {
		err = fmt.Errorf("gommod.Initialize: %w", err)
		goto end
	}
	initializeLoggers(args.Logger)

	if args.Writer == nil {
		err = errors.New("gommod.Initialize: Writer not set")
		goto end
	}
	err = cliutil.Initialize(args.Writer)
	if args.Writer == nil {
		err = fmt.Errorf("gommod.Initialize: clituil.Initialize() failed; %w", err)
		goto end
	}
