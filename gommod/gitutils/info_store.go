...
func NewInfoStore(dirPath dt.DirPath, filename dt.RelFilepath) *InfoStore {
	return &InfoStore{DirPath: dirPath, Filename: filename}
}
...
func (s *InfoStore) GetDirPath() (dp dt.DirPath, err error) {
	var exists bool
	dp = dt.DirPathJoin(s.DirPath, InfoPath)
