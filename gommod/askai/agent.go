...
// If no provider is specified, defaults to ClaudeCLIProvider with "claude" executable
func NewAgent(args AgentArgs) *Agent {
	...
	if provider == nil {
		// Default to Claude CLI provider
		provider = NewClaudeCLIProvider(DefaultClaudeCLIProviderArgs())
	}
