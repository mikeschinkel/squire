...
func NewCommitMessageMode(args CommitMessageModeArgs) *climenu.BaseMenuMode {
	...
	options := []climenu.MenuOption{
		...
		},
		{
			...
			Handler: func(handlerArgs *climenu.OptionHandlerArgs) error {
				newMessage, err := EditMessage(*args.Message, args.Writer)
				if err == nil {
		...
		},
		{
			...
			Handler: func(handlerArgs *climenu.OptionHandlerArgs) error {
				...
				newMessage, err := gompkg.RegenerateMessage(context.Background(), args.ModuleDir, args.AnalysisResults, agent, args.Writer.Writer())
				if err == nil {
					*args.Message = newMessage
				}
				return err
	...
	if args.AnalysisResults != nil {
		options = append(options,
			climenu.MenuOption{
				...
				Handler: func(handlerArgs *climenu.OptionHandlerArgs) error {
					DisplayAnalysisReport(args.AnalysisResults, args.Writer)
					return nil
				},
			},
			climenu.MenuOption{
...
func generateCommitMessageInteractive(moduleDir dt.DirPath, writer cliutil.Writer, logger *slog.Logger) (err error) {
	...
	message, analysisResults, err = gompkg.GenerateWithAnalysis(ctx, gompkg.GenerateWithAnalysisArgs{
		ModuleDir: moduleDir,
		Logger:    logger,
...
func handleMultiCommitFlowInteractive(moduleDir dt.DirPath, analysisResults *precommit.Results, writer cliutil.Writer) (err error) {
	...
	DisplayCommitGroups(groups, writer)
