...
func (r APIDiffReport) HasBreakingChanges() bool {
...
func (r APIDiffReport) WritePackageChanges(w io.Writer) {
	if r.LoadErrors != nil {
...
func (r APIDiffReport) write(w io.Writer, msgs ...any) {
	switch len(msgs) {
	case 0:
	case 1:
		_, _ = fmt.Fprint(w, msgs[0].(string))
	default:
		dtx.Fprintf(w, msgs[0].(string), msgs[1:]...)
	}
}

func (r APIDiffReport) writeList(w io.Writer, label string, items []string) {
	if len(items) == 0 {
		return
	}
	sort.Strings(items)
