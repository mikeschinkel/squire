# Squire CLI – v0 Product Requirements Document (PRD)

## 1. Purpose and Scope (v0)

This PRD defines the **v0 scope** for the Squire CLI:

- Provide a **repeatable, automatable way to discover Go modules** under one or more filesystem roots.
- Generate **repo-local configuration** in `.squire/config.json` that records which modules exist in each repo and what high-level role each plays.
...
- **Root path / root** – A filesystem directory under which Squire should search for repos and modules.
- **Managed repo** – A repo that has a `.squire/config.json` file at its repo root.
- **Unmanaged repo** – A repo with Go modules but no `.squire/config.json`.


## 3. Goals (v0)

1. **Discover Go modules across the filesystem** under user-specified roots, grouped by repo.
2. **Identify unmanaged repos** (those without `.squire/config.json`) and expose their module `go.mod` paths.
3. **Generate minimal repo config** in `.squire/config.json` that records:
...
4. Support **batch onboarding** of existing repos via a file-based workflow:
   - `squire scan` → text file of candidate modules.
   - User edits file to choose which modules/repos to adopt.
   - `squire init --file <path>` → writes `.squire/config.json` for those repos.
...
- Any modification of user code or `go.mod` other than creating `.squire/config.json`.


## 5. Assumptions and Constraints

- Squire v0 is **Go-centric**: modules are identified via `go.mod` files.
...
- Config that affects CI/CD behavior **must live in the repo**, under `.squire/config.json`. Machine-level state is purely ergonomic/caching and not required by CI.
...
- Repo-local config lives at:
  ...
  - `<repo-root>/.squire/config.json`

- The `.squire` directory is created by Squire as needed.
...
- **Key**: repo-relative path to the module directory, using `./` notation:
  ...
  - Submodules under `cmd/` → e.g., `"./cmd"` or `"./cmd/squire-cli"` (depending on layout).
  - Submodules under `test/` → e.g., `"./test"`.
- **Value**: module metadata object with fields:

  - `name` (string; required)
    - Short, human-friendly name for the module.
    - Must be **unique per repo** for modules Squire is expected to manage.
...
- **Validation rules (v0):**
  - If `.squire/config.json` exists, Squire v0 should:
    - Validate JSON syntax.
    - Validate `modules` is an object.
    - Validate each module’s `name` is a string and `role` is a non-empty array of strings.
    - Ensure `name` values are unique per repo for Squire-managed modules.
  - v0 may treat unrecognized fields as opaque and ignore them (to ease future schema evolution).


## 7. CLI Commands (v0)

v0 defines two core commands: `squire scan` and `squire init`.

### 7.1 `squire scan`

**Purpose:** Discover Go modules under one or more roots, restricted to repos that do **not** yet have `.squire/config.json`. Output the full filesystem path to each `go.mod` on its own line.
...
2. When a `go.mod` is found:
   ...
   - Check whether `<repo-root>/.squire/config.json` exists.
   - If `.squire/config.json` **exists**, skip this `go.mod` (repo is already managed).
...
**Purpose:** Create `.squire/config.json` files for repos that do not already have one, based on a curated list of `go.mod` paths.

v0 focuses on a **batch adoption mode** driven by a file. A single-repo `squire init` (run from within a repo) may be added later but is not required for v0.
...
4. For each repo root group:
   - If `<repo-root>/.squire/config.json` already exists:
   ...
   - Otherwise:
     ...
     - Infer `role` values using simple heuristics:
       ...
       - All others → `"lib"`.
     - Construct a `modules` map and write it to `<repo-root>/.squire/config.json`.
       - Create the `.squire` directory if it does not exist.
5. Report a summary:
   - Number of repos initialized.
   - Number of repos skipped due to existing `.squire/config.json`.
...
- If a repo already has `.squire/config.json`, log a warning indicating it was skipped.
...
# Initialize .squire/config.json for those repos
squire init --file init.txt
```


## 8. User Stories (v0)

1. **As a developer with many existing Go repos**, I want to run a single command to discover all repos/modules that are not yet Squire-managed so I can decide which to onboard.

2. **As a developer**, I want to edit a simple text file listing module `go.mod` paths and then have Squire create `.squire/config.json` files for all selected repos in one batch operation.

3. **As a developer**, I want Squire to infer reasonable defaults for module names and roles, so I do not need to hand-author JSON for each repo.

4. **As a CI maintainer**, I want `.squire/config.json` to live inside each repo and fully describe its modules/roles for v0 features, so CI jobs can run Squire without any dependence on machine-local configuration.
...
- Should Squire allow a single-repo `squire init` (no `--file`) that infers modules from the current directory’s repo?
- Should scan support additional filters (e.g., ignore repos based on a user-local ignore list in `~/.config/squire/`)?
...
- Whether to allow `.squire/config.json` to be created in non-Go repos for future multi-language support.
