# Gomion — Multi-Purpose CLI Tool for Go-centric Developers

## Features, Requirements, Background, and Design Notes

This document captures the early requirements, background, and design decisions for **Gomion**, a Go-centric developer CLI that centralizes project tooling and workflows.
...
- Provides an opinionated but configurable layer around:
  - Testing (`gomion go test`)
  - Linting (`gomion go lint`)
  - Building & releasing (`gomion go build` + GoReleaser scaffolding)
  - Fuzzing (`gomion go fuzz`, inspired by the existing infinite fuzz script)
...
## 2. Core problems Gomion is meant to solve
...
- Gomion understands **owned modules** per:
  - workspace,
  - repo, and
  - module.
...
- Gomion will eventually provide a **ClearPath linter** (e.g. `gomion go lint --style=clearpath`) that can be run:
  - locally,
  - in CI,
...
- ClearPath is first-class in Gomion’s docs and design, not an afterthought.
...
Gomion’s design incorporates that:

- Gomion will subsume the doterr CLI’s responsibilities:
...
- The **doterr use-case** is the canonical example of Gomion’s “non-dependency dependency” support, but the pattern should be general enough to apply to other single-file tools in the future.
...
- For **Go experiments** and other low-level build toggles:
  ...
  - Gomion parses those and sets `GOEXPERIMENT` when running `go` commands, fuzzing, etc.
  - `.gomion` config should not become a second, conflicting “watch” for the same information.

- For **doterr** and similar embed tools:
  - Use `.gomion/gommod/gomion.json` instead of `doterr.json` as the canonical source for which packages use `copy` vs `dot` vs `ignore`.
  - Generated files and `go:generate` directives follow from that.

- For **workspaces**:
  - User-level config (e.g. `~/.config/gomion/config.json`) is the canonical source of:
    - Workspace names and roots.
    - Owned-module patterns.
    - Possibly the “current” workspace.
  - Workspace root `go.work` files are the concrete representation for the Go tool, but Gomion shouldn’t require you to maintain both manually.

Design note: **.gomion vs inline config**

- `.gomion` (directory or file) is expected to hold Gomion-specific metadata and state.
- Inline directives (like `//gomion:goexperiments=...`) live in code and are treated as the canonical configuration where they exist.
- Gomion must be very deliberate about:
  - Which things are configured via `.gomion`, and
  - Which things are configured via inline comments.

The overarching guideline: **pick one primary place per concern, and have Gomion keep derived views in sync.**
...
- Gomion should support **user-level workspace config**, likely under `~/.config/gomion/…`, listing:
  - Workspaces and their root directories.
  - Patterns for “owned” module paths.
- In each repo, Gomion should support a **.gomion** directory (e.g. `.gomion/gommod/gomion.json`) for repo/module-specific metadata (role, release behavior, etc.), but **not** for things that can be reliably inferred from `go.mod` or inline directives.
- Gomion must always be able to discover:
  - Repos via `.git`.
  - Modules via `go.mod`.
- Gomion should expose `gomion workspace` commands, including:
  - `gomion workspace discover` with a **TUI** to scan known roots, group repos into workspaces, and adjust settings interactively.
  - `gomion workspace use <name>` to set the current workspace.

---

## 5. Interactive TUI requirements

Some parts of Gomion are **inherently interactive** and should be implemented as TUIs:

1. **Workspace discovery (`gomion workspace discover`)**
...
## 6. Go-specific command layer (`gomion go`)

Gomion’s Go backend will provide a family of subcommands (names are not final):
...
- `gomion go build`
  - Builds CLI modules (`role: app`) according to Gomion’s conventions.
...
- **Tagging and releasing** should be done via **GitHub Actions**, not via Gomion directly.
  - Gomion can generate or update workflows, but the actual tag/release happens in CI.
- The **release workflow** should:
  - Run `gomion go test` / lint / vet first.
  - Only tag & release on success.
  - Use GoReleaser for binary releases when appropriate.

Gomion should provide:

- `gomion go release init`
  - TUI to configure:
...
- Optionally:
  - `gomion go release --local` to run the Release flow locally for sanity before pushing tags.

---

## 8. Multi-language future: “current language”

Even though Go is the primary focus, the design should leave room for other languages (e.g. Zig) to reuse Gomion’s concepts.
...
* `gomion test` and similar commands:

    * Default to `currentLanguage`.
    * Allow overrides: `gomion test --lang=zig` or shorthand like `--zig`.
* Internally:
...
Gomion should eventually help scaffold:

* **Go CLI projects**, using your own `go-cliutil` instead of Cobra/urfave-cli.

    * `gomion go new-cli <name>`:

        * Creates `cmd/<name>/main.go` with a `RunCLI()` pattern.
        * Sets up `go.mod` if needed.
        * Optionally creates `.gomion` config marking the module as `role: app` and enabling GoReleaser scaffolding.
...
* **Template-based project layouts**:
    ...
    * Gomion should avoid duplicating this – templates should be the primary source of truth.
...
* You previously designed a tool for **license-based module governance** (e.g. allow Apache-2.0, forbid GPL). That could be another Gomion module in the future.

Gomion should keep these in mind as **future features**:

* A policy layer in `.gomion` (per workspace/org) that:

    * Expresses license and dependency rules.
    * Optionally generates or populates configs for tools like gomodguard / golangci-lint.
* CI integration via `gomion go deps check` or similar.

---

## 11. Integration with existing tools & libraries

Gomion is intended to sit on top of, not replace:

* `go` command and standard tooling.
* `GoReleaser` for binaries.
* `goyek` for task automation (Gomion can expose reusable Go APIs that goyek flows call).
* `go-doterr` for embeddable error handling.
* Existing one-off tools (`ssot`, `readme-merge`) which can be pulled into Gomion as subcommands over time.

Design preferences:

* Gomion should be usable both as:

    * A **CLI** (`cmd/gomion`), and
...
2. **Canonical location for certain configs**:
    ...
    * Experiments: inline comment in `go.mod` vs `.gomion`.
    * How much Gomion metadata (e.g. roles, release settings) belongs in `.gomion` vs other files.

3. **How aggressively to adopt/normalize existing `replace` directives**:

    * Default stance is conservative: only touch Gomion-tagged sections.
