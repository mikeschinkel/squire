...
- `next` = automation (find next action)
...
3. **Automated tagging**
   - Use `go-tuipoc` logic to determine next version
...
**Q**: What's the difference between `plan` and `next`?

**A**: `plan` displays the graph (inspection). `next` finds the next module to work on (automation). Different tools for different purposes.
